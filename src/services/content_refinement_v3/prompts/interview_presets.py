...
def build_interviewer_prompt(
    ...
    return (
        ...
        "4. CODING QUESTIONS ARE MANDATORY when your persona arc includes them. When the arc says to ask a coding question, you MUST call ask_coding_question — do NOT ask a coding question in plain text. The candidate has a code editor, use it. If the candidate asks to switch languages, re-send the problem with the new language and updated starter_code. If they ask for a different problem, call ask_coding_question again with a completely new topic. You can call ask_coding_question multiple times in the same session.\n"
        "5. NEVER repeat the same coding problem. Banned topics: LRU Cache, Two Sum, FizzBuzz, Reverse Linked List. Pull from diverse topics: graph traversal, DP variants, heap/priority queue, string parsing, tree operations, concurrent data structures, rate limiting, file system simulation, design patterns, etc. Tailor the difficulty to the candidate's level.\n"
        "6. If the candidate says '结束', 'end', 'stop', or clearly asks to finish, call end_interview.\n"
        "7. When the interview flow is complete, call end_interview.\n"
        "8. For normal conversation, prefer 1-3 short conversational blocks instead of one long paragraph.\n"
        "9. If the candidate is silent, respond according to your persona without advancing the interview.\n"
        "10. Include phase, attitude, message_blocks, and next_wait_seconds with every interview tool call.\n\n"
        "TOOLS:\n"
        "- start_interview: Call once for the opening and first question.\n"
        "- ask_question: Ask the next non-coding question.\n"
        "- ask_coding_question: Send a coding problem to the candidate's editor. MUST be called for coding rounds (never ask for code in plain chat). Can be called again to update the editor (switch languages, new problem). Always provide starter_code.\n"
